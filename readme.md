...
更新（2026-1-25）：调LQR后需重新调跳跃时更换的pid，限幅不用改，调到能够原地落地也能平稳即可。

- 倒地自启

原小腿判断方案，~~或者简单pid收腿至稳态后起立~~。

LQR K阵不需要切换即可，注意`status`延缓时间决定连续性。
...
更新（2026-1-25）：中石油方案可行，但卡尔曼过大导致会向后走。
...
| 名称            | 原   | 现   | 对比                     |
| --------------- | ---- | ---- | ------------------------ |
| 腿长            | 单环 | 双环 | 似乎没啥区别，可能好调了 |
| $\delta \theta$ | 单环 | 单环 | **TODO**                 |

### 问题解决

- 遥控问题（框架问题）

  莫名奇妙断点后就接收不到了，并且跳跃后也接收不到（应该是瞬间松了）

  - 不要在 IRQHandler 里手动调用 HAL_UARTEx_RxEventCallback，也不要在 IRQ 里重复调用 HAL_UARTEx_ReceiveToIdle_DMA。HAL_UART_IRQHandler 会在合适时机触发 HAL 层回调并由回调重启 DMA。手动调用会破坏 HAL 内部状态/长度，导致断点后或虚连/重连后不能再进中断。
...
void UART5_IRQHandler(void)
{
  ...
  // Removed manual call to HAL_UARTEx_RxEventCallback and manual restart.
