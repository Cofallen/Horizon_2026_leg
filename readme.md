...
~~减小轮子R，即增加其扭矩输出后，发现只会抖动，不会移动，因此猜测是力矩在低电流输出下的效果不好。默认必须减速比加上19：1的项，否则物理参数感觉不对。~~ 默认手册参数是输出轴转矩常数，除以19是对的。

~~待测试低扭矩的死区设置，目前打算按照0.5下按照e指数衰减测试。~~已测试阶跃死区设置，并不能解决问题，我猜测可能和sim2real gap有关。

更新（4.19）：猜测提高轮子控制频率至1khz.
...
这个规划一下腿的旋转逻辑即可。~~目前计划舍弃PID方案（因为效果参数太难测试）~~ 计划将左右腿轨迹变化设置成一样的后，加上防劈叉，这样简单一点（也许）

- [ ] FDCAN 以及 ERR Handle

H7目前需要外挂设备，CAN发送才能成功，否则进入ERR Handle，另外陀螺仪也必须24V接入，否则也是。 待查明代码位置。
...
测试单纯补偿，将输出加到支持力上，通过vmc解算到髋关节即可，需注意应叠加一项腿的重力。

其他动态使用中，直接加到输出力即可，方向应与你设定的支持力方向相同（即增加了绝对值）。
...
- 2026-3-31 项目被砍了，上不了场。计划多探索一些算法仿真和部署验证，暂时不考虑稳定性了。

- 2026-3-31 还是很难过的，毕竟算来11月份开始，在上面投入的精力也有个5个月了，说扔就扔。先这样吧。

- 2026-4-19 测试theta变化和走线，观察提高频率后是否仍有走动问题。
