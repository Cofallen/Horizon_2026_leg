...
{   
    ALL_MOTOR.right_front.DATA.pos_init_rad = -1.7247f;
    ALL_MOTOR.right_back.DATA.pos_init_rad  = -0.3268f;
...
{   
    ...
    object->target.l0 +=  ((float)dbus->Remote.CH1_int16 / 660000.0f + (float)(dbus->KeyBoard.Q - dbus->KeyBoard.E) * 0.0001f); 
