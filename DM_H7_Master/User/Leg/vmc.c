...
void Vmc_Init(Leg_Typedef *object, float target_l0)
{
    ...
    const float Roll_control[3] = {2.0f, 0.0f, 0.0f};

    PID_init(&object->pid.F0_l, PID_POSITION, F0_control, 60.0f, 0.0f);
    PID_init(&object->pid.Yaw, PID_POSITION, Yaw_control, 1.0f, 0.0f);
    PID_init(&object->pid.Delta, PID_POSITION, Delta_control, 0.0f, 0.0f);
